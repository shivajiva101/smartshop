		...
		elseif not pressed.quit then
			...
			if pressed["buy" .. n] then
				...
				if name ~= "" then
					...
					inv:contains_item("give" .. n, stack) == true then
						inv:add_item("main", stack)
						inv:remove_item("give" .. n, stack)
					end
					if type == 1 and inv:contains_item("main", stack) == false then
						minetest.chat_send_player(pname, "Error: Out of stock!")
						return
					end
			...
			if smartshop.user[player:get_player_name()] or
...
--clear
	...
	for _, ob in ipairs(minetest.env:get_objects_inside_radius(pos, 2)) do
		if ob and ob:get_luaentity() and ob:get_luaentity().smartshop and
		ob:get_luaentity().pos == spos then
			ob:remove()	
		end
	end
...
--update
	...
	for i = 1,4,1 do
		...
		if item ~= "" then
			...
			{x=pos.x+pos2.x,y=pos.y+pos2.y,z=pos.z+pos2.z},
			"smartshop:item")
...
smartshop.showform = function(pos,player,re)
	...
	if owner then
		if meta:get_int("type") == 0 and not (minetest.check_player_privs(uname, {creative=true}) or
		minetest.check_player_privs(uname, {give=true})) then
...
after_place_node = function(pos, placer)
		...
		meta:set_int("type",1)
		meta:set_int("sellall",1)
		if minetest.check_player_privs(placer:get_player_name(), {creative=true}) or
		minetest.check_player_privs(placer:get_player_name(), {give=true}) then
...
can_dig = function(pos, player)
		...
		minetest.check_player_privs(player:get_player_name(), {protection_bypass=true})) and
		inv:is_empty("main") and inv:is_empty("pay1") and inv:is_empty("pay2") and
		inv:is_empty("pay3") and inv:is_empty("pay4") and inv:is_empty("give1") and
		inv:is_empty("give2") and inv:is_empty("give3") and inv:is_empty("give4")) or
